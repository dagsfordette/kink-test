...
export function normalizeResponsePayload(catalog, data, { allowMissingFormat = true } = {}) {
  if (!data || typeof data !== 'object') throw new Error('This JSON does not look like an exploration export.')
  if (data.questionnaireId !== catalog.questionnaire.id) throw new Error('This JSON belongs to a different questionnaire.')
  if (!allowMissingFormat && data.format !== RESPONSE_FORMAT) throw new Error('Unsupported exploration response format.')
