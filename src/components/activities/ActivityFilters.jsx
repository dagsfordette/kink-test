  ...
  return (
    <section className="activity-filter-panel no-print" aria-label="Activity filters">
      <div className="activity-search-row">
        ...
        <button type="button" className="secondary-button compact" onClick={onBrowseEverything}>Show everything</button>
      </div>
      ...
      <label className="activity-show-hidden"><input type="checkbox" checked={navigation.showHidden} onChange={(event) => onChange({ showHidden: event.target.checked })} /> Show hidden / skipped things</label>
    </section>
  )
}
